...
const WalkController = (function () {
    ...
    function WalkController(config) {
        ...
        self.KEY = {
            CANCEL: KEY.ESC,
            CTRL: KEY.CTRL,
            MOVE_TO_EYE_HEIGHT: KEY.GRAVE_ACCENT,
            MOVE_FORWARD: KEY.ARROW_UP,
            MOVE_LEFT: KEY.ARROW_LEFT,
            MOVE_BACK: KEY.ARROW_DOWN,
    ...
    WalkController.prototype.onKeyDown = function (event) {
        ...
        switch (event.keyCode) {
            case KEY.CTRL:
                self.modifiers[KEY.CTRL] = true;
                break;
            case self.KEY.MOVE_TO_EYE_HEIGHT:
                self.setWalkHeight();
                self.dispatchEvent({type: EVENT.CONTINUOUS_UPDATE_START});
                break;
    ...
    WalkController.prototype.onKeyUp = function (event) {
        ...
        switch (event.keyCode) {
            case KEY.CTRL:
                self.modifiers[KEY.CTRL] = false;
                break;
            case self.KEY.MOVE_FORWARD:
                self.move.forward = false;
                self.dispatchEvent({type: EVENT.CONTINUOUS_UPDATE_END});
                break;
            case self.KEY.MOVE_BACK:
    ...
    WalkController.prototype.update = function (delta) {
        ...
        if (change) {
            self.dispatchEvent({type: EVENT.RENDER});
        }
    };
