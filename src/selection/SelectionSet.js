   ...
   * }
    ...
    SelectionSet.prototype.add = function (obj, update) {
        ...
        if (this.index.indexOf(id) === -1) {
            ...
            if (update) {
                this.updateIndex();
                this.dispatchEvent({type: EVENT.UPDATE, added: [obj], removed: [], selected: this.items});
            }
        }
    };
    ...
    SelectionSet.prototype.addAll = function (objects, update) {
        ...
        if (update) {
            self.updateIndex();
            self.dispatchEvent({type: EVENT.UPDATE, added: objects, removed: [], selected: self.items});
        }
    };
    ...
    SelectionSet.prototype.remove = function (obj, update) {
        ...
        if (update) {
            this.updateIndex();
            this.dispatchEvent({type: EVENT.UPDATE, added: [], removed: removed, selected: this.items});
        }
        return removed;
    };
    ...
    SelectionSet.prototype.select = function (selection) {
        ...
        this.dispatchEvent({type: EVENT.UPDATE, added: added, removed: removed, selected: this.items});
    };
