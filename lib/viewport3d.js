...
FOUR.Viewport3D = (function () {
    ...
    Viewport3D.prototype.animate = function () {
        var self = this;
        var delta = self.clock.getDelta();
        requestAnimationFrame(self.animate);
        Object.keys(self.MODES).forEach(function (key) {
           if (self.mode === self.MODES[key] &&
               self.controller[self.MODES[key]].hasOwnProperty('update')
           ) {
               self.controller[self.MODES[key]].update(delta);
           }
        });
