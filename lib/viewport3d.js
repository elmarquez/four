...
var Viewport3D = (function () {
    ...
    Viewport3D.prototype.getViewBoundingBox = function () {
        ...
        if (self.scene.selection.count > 0) {
            ...
            var bbox = new BoundingBox('scene-bounding-box');
            bbox.update(self.scene.children); // FIXME filter for model entities only
            return bbox;
        }
    };
    ...
    Viewport3D.prototype.setupKeyboardBindings = function () {
        var self = this;
        // modifier keys
        // TODO set selection controller modifier value too
        ...
        Mousetrap.bind('ctrl+a', function () {
            ...
            var filter = function (obj) {
                // filter out non-model entities
                if (obj instanceof THREE.BoxHelper ||
                    obj instanceof THREE.Camera ||
                    obj instanceof THREE.Light ||
                    obj instanceof THREE.Scene ||
                    obj instanceof THREE.TransformGizmo ||
                    obj instanceof BoundingBox) {
                    return false;
                }
                return true;
            };
            self.controller.selection.selectByFilter(filter);
        });
